...
def plot_components(ax, nu, components, label_prefix, show_legend=True, alpha=0.5):
    ...
    for i, component in enumerate(components):
        if show_legend:
            ax.plot(nu / 1e9, component, linestyle=component_lines[i], label=label_prefix +
                    component_names[i], color=component_colors[i], alpha=alpha)
        else:
            ax.plot(nu / 1e9, component,
                    linestyle=component_lines[i], color=component_colors[i], alpha=alpha)


def plot_components_confidence_intervals(ax, nu, components_samples, sigmas=1):
    component_colors = ['b', '#5ebaf2',   'violet',  'sandybrown', 'grey'] # Match with plot_components
    ...
    for band_name in bands_to_plot:
        if band_name in bands_dict:
            ...
            ax.text(band['start'] + band['width'] / 2 + 0.9, band['pos'], band_name, color=band['color'],
                    ha='right', va='center', rotation='vertical', fontsize=16)
